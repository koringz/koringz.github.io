            ...
            */
            ...
            test : function () {
                for(var i = 0; i <tlength; i++){
                    ...
                    // 假定度数为 i = 0 ~ 10 之间
                    // i 的变化为半个周期 
                    // 也就是说半个周期的度数约在0 ~ 180度之间
                    // 那么i的MAX最大数为10 所以传入的参数最大为18
                    // 如果传入一个时间轴 需要设定最大约数为18
                    // 所以参数为小于等于18 || <= 18
                    // 于是0~10之间有10个数 那么18就要进行一次10等分 所以结果就是每次传入 18
                    // add - 由于时间轴数字比较大 故每秒的位移随着角度发生变化 可以把时间轴放到弧度上相加
                    var radian = i * nums * Math.PI / 180  * times;

                    var radianValues = Math.sin(radian);

                    // 根据纵坐标的位移实现几何图形 余弦
                    // 使用的是n的平方根
                    var y = Math.pow(radianValues*2,2);

                    // 由于每秒运行的位移必须要加上时间轴 step
                    // 那么step一直在增加时 位移也会相应的改变 
                    // 也就是说位移随着时间的流逝 事物会不断地在运动当中
                    // 然而时间轴以毫秒来计算的 数字比较大
                    // 所以我们会把这个step时间轴放到角度里面去
                    // 用于计算每秒的弧度变化效果
                    // 然后再把这个弧度数放到位移上
                    // 那么就可以看到这个事物一直在变化和运动当中
                    var x = i;
