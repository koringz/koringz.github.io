			...
			}([
				{
						...
						window.msRequestAnimationFrame || (function (){
							...
							return function (callback){
								...
								var t = setTimeout(function (callback) {
									callback(currentTimes + currentTimes);	
								},callTimes);
					...
					3 : function  () {
						...
						o = [
							...
							{ 
								1: 'get', 2: 'set', 3:'add', 66: 'on'
							},  /*prototype chain*/
							{ 
								1: 'click', 3: 'mouseover', 6: 'gesture', 11: 'atach'
							}  /*gesture events*/
						]
					}
				},
					...
					99 : function  () {
						...
						for (var k in data){
							...
							if(typeof inherit === 'string' || typeof inherit === 'number' || typeof inherit === 'boolean' ){
								...
								this.defaults[k] = {name:'withoutMethod',type:typeof data[k]}
							}
							else if( typeof inherit === 'function' ){
								this.newEmpty.push(k);
								this.defaults[k] = {name:'withMethod',type:typeof data[k],length:data[k].length}
							}
						}
		...
		*/
		...
		Object.assign(o[1][99].prototype,{
			...
			createGradient20 : function  (argument) {
				var rgb = ['#5E8579','#7B7687','#77C34F','#96CDCD','#407D94','#D62728','#C1FFC1','#2E8B57','#87CEEB','#9ACD32',
				'#DFB5B7','#EACF02','#ACB327','#7F1784','#E08031','#C7CEB2','#199475','#0B6E48','#044D22','#7E8842'
				];
				return rgb
			},
					...
					graphics : {
						...
						config : {
							set :(function (){
								...
								// 执行默认的参数类型
								// 只能设置到[]数组的第一层
								var atpc = new o[1][83]({type:'Array',property:[]});
								// 重新set设置参数的类型和属性
					...
					return {
						...
						*/
						addChild :function (options) {
							...
							// 重新定义type 
							var acceptCustomizeConfig = Object.assign(defineConfigurationInfo,{type: defineConfigurationInfoPropertiesType})
			...
			draw : function (options){
				...
				for ( var j =0; j< acceptCollection.length; j++){
					...
					for(var i = beginDistance_x; i< endDistance_x; i++){
						...
						this._lineTo({x:this.iterator.dynimic.x,y:this.iterator.dynimic.y});
					}
